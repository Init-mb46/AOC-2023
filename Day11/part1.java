        ...
        for (int i = 0; i < galaxies.size(); i++) {
            for (int j = i + 1; j < galaxies.size(); j++) {
                ...
                for (int r : rowsExpansion) {
                    if ((r > Integer.parseInt(xyA[0]) && r < Integer.parseInt(xyB[0])) || (r < Integer.parseInt(xyA[0]) && r > Integer.parseInt(xyB[0]))) add += 1;
                }
                for (int c : columnsExpansion) {
                    if ((c > Integer.parseInt(xyA[1]) && c < Integer.parseInt(xyB[1])) || (c < Integer.parseInt(xyA[1]) && c > Integer.parseInt(xyB[1]))) add += 1;
                }
