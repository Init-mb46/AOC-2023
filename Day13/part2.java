...
public class part2 {
    ...
    public static int process(ArrayList<String> pattern) {
        ...
        for (int i = 1; i < pattern.get(0).length(); i++) {
            ...
            if (errors == 1 && (i - max == 0 || i + max == pattern.get(0).length()) && max > maxRef) {
                maxRef = max;
                maxRefPos = i;
            }
        }
        ...
        for (int i = 1; i < pattern.size(); i++) {
            ...
            for (int j = 0; j < pattern.get(i).length(); j++) {
                ...
                for (int k = 0; k <= i; k++) {
                    ...
                    if (pattern.get(i-k-1).charAt(j) != pattern.get(i+k).charAt(j)) {
                        errors++;
                    }
                    locMax++;
                }
            ...
            if (errors == 1 && (i - max == 0 || i + max == pattern.size()) && max > maxRef) {
                row = true;
                maxRef = max;
                maxRefPos = i;
            }
